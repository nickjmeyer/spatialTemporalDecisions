...
dataDir = "../../data/wns/2016-11-04-11-57-19/"
...
for(f in files) {
  if (grepl("^rank_M1Sp_history",f)) {
    h = as.matrix(read.table(paste(dataDir,f,sep=""),header=FALSE))[timeStep,]
    rankData = rbind(rankData, h)
  } else if (grepl("^myopic_history",f)) {
    h = as.matrix(read.table(paste(dataDir,f,sep=""),header=FALSE))[timeStep,]
    myopicData = rbind(myopicData, h)
  }
}

rankTrt = colMeans(rankData %% 2)
rankInf = colMeans((rankData < 2) + 0)
