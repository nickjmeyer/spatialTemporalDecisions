...
std::vector<double> WnsFeatures1TuneParam::getPar() const{
  return std::vector<double>(0);
}


void WnsFeatures1TuneParam::putPar(const std::vector<double> & par){
  // do nothing!
}
...
template <class M>
void WnsFeatures1<M>::preCompData(const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
...
template <class M>
void WnsFeatures1<M>::getFeatures(const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
				  const DynamicData & dD,
				  M & m){
...
template <class M>
void WnsFeatures1<M>::updateFeatures(const SimData & sD,
				     const TrtData & tD,
				     const FixedData & fD,
				     const DynamicData & dD,
				     M & m){
