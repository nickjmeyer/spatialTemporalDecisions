...
double GravityTimeInfLogModel::tuneTrt(const FixedData & fD,
				       const GravityTimeInfLogParam & gP){
  int i,j;
  double avgCaves = 0.0;
...
double
GravityTimeInfLogModel::oneOnOne(const int notNode,
				 const int infNode,
				 const SimData & sD,
				 const TrtData & tD,
				 const FixedData & fD,
				 ...
				 const
				 GravityTimeInfLogParam & gP) const{
  double base = gP.intcp;
  ...
  base += gP.xi * std::log(sD.timeInf.at(infNode));
...
void GravityTimeInfLogModel::fit(const SimData & sD, const TrtData & tD,
				 const FixedData & fD, const DynamicData & dD,
				 GravityTimeInfLogParam & mP){
  GravityTimeInfLogParam mPInit;
...
void GravityTimeInfLogModel::fit(const SimData & sD, const TrtData & tD,
				 const FixedData & fD, const DynamicData & dD,
				 GravityTimeInfLogParam & mP,
				 const GravityTimeInfLogParam mPInit){
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfLogModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfLogModelFitObjFn;
...
double gravityTimeInfLogModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfLogModelFitData * dat =
    static_cast<GravityTimeInfLogModelFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=dat->mP.xi*std::log(dat->timeInf.at(t-1).at(j));
