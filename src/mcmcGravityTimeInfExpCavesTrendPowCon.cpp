...
void GravityTimeInfExpCavesTrendPowConSamples::setMean(){
  ...
  trendPowConSet = std::accumulate(trendPowCon.begin(),trendPowCon.end(),0.0);
  trendPowConSet /= double(numSamples);
...
void GravityTimeInfExpCavesTrendPowConSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
  ...
  powerSet = power.at(i);
  trendSet = trend.at(i);
  trendPowConSet = trendPowCon.at(i);
  xiSet = xi.at(i);
...
void GravityTimeInfExpCavesTrendPowConSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
if(fromBurn){
  ...
  trendPowConSet = trendPowConBurn.at(i);
  xiSet = xiBurn.at(i);
...
std::vector<double> GravityTimeInfExpCavesTrendPowConSamples::getPar() const {
...
void
GravityTimeInfExpCavesTrendPowConMcmc
...
::load(const std::vector<std::vector<int> > & history,
  ...
  for(i = 0; i < numNodes; ++i){
	...
	++val;
      timeInfVal = std::exp((val - 1.0)/fD.propCaves.at(i)) - 1.0;
...
void
GravityTimeInfExpCavesTrendPowConMcmc::sample(int const numSamples,
					      int const numBurn,
const bool saveBurn){
...
void GravityTimeInfExpCavesTrendPowConMcmc
...
samples.numBurn = numBurn;
  ...
  double intcp_mean=0,intcp_var=100,beta_mean=0,beta_var=10,alpha_mean=0,
    ...
    trendPowCon_mean=0,trendPowCon_var=1,
  ...
  trendPowCon_cur=trendPowCon_can=*it++;
  ++it;
...
samples.trendBurn.reserve(numBurn);
  ...
  samples.trendPowCon.clear();
samples.trendPowConBurn.clear();
  samples.trendPowCon.reserve(numSamples-numBurn);
samples.trendPowConBurn.reserve(numBurn);
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("GravityTimeInfExpCavesTrendPowCon...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
    ...
    // sample trendPowCon
    ++att.at(numCovar+TRENDPOWCON_);
    upd=trendPowCon_cur+mh.at(numCovar+TRENDPOWCON_)*njm::rnorm01();
    trendPowCon_can=upd;
    ...
    R=ll_can + (-.5/trendPowCon_var)*std::pow(trendPowCon_can
					      - trendPowCon_mean,2.0)
      - ll_cur - (-.5/trendPowCon_var)*std::pow(trendPowCon_cur
						- trendPowCon_mean,2.0);
      
    
    // accept?
    if(std::log(njm::runif01()) < R){
      ++acc.at(numCovar+TRENDPOWCON_);
      trendPowCon_cur=trendPowCon_can;
      ll_cur=ll_can;
    }
    else{
      trendPowCon_can=trendPowCon_cur;
      ll_can=ll_cur;
    }
    ...
    if(std::log(njm::runif01()) < R){
      ++acc.at(numCovar+XI_);
      xi_cur=xi_can;
      xiTimeInfExpCaves_cur = xiTimeInfExpCaves_can;
      ll_cur=ll_can;
    }
    else{
      xi_can=xi_cur;
...
beta_cur.end());
      ...
      samples.trendPowConBurn.push_back(trendPowCon_cur);
      ...
      samples.trendPowCon.push_back(trendPowCon_cur);
  ...
  trendPowCon_can = samples.trendPowConSet;
  ...
  xiTimeInfExpCaves_can = timeInfExpCaves;
  updateXiTimeInfExpCaves(xiTimeInfExpCaves_can,xi_can);
...
double GravityTimeInfExpCavesTrendPowConMcmc::ll(){
  int i,j,k;
  ...
  for(i=1; i<T; i++){// loop over time interval that has changed
	...
	for(k=0; k<numNodes; k++){
	  ...
	  if(infHist.at(k*T + i-1)==1){
	    ...
	    // i is time + 1
	    baseProb += trend_can*std::pow(double(i), 
					   -std::exp(trendPowCon_can));
