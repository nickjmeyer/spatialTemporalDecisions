...
static std::vector<ParamBase *> genPars(){
  ...
  pars.push_back(new ParamGDistPow);
  pars.push_back(new ParamTrt);
  return pars;
}

ModelGDistPow::ModelGDistPow(const FixedData & fD)
  : ModelBase("GDistPow",genPars(),fD){
}


ModelGDistPow::ModelGDistPow(const ModelGDistPow & m){
...
ModelGDistPow & ModelGDistPow::operator=(const ModelGDistPow & m){
  if(this != & m){
    this->ModelGDistPow::~ModelGDistPow();
    new (this) ModelGDistPow(m);
  }
  return *this;
}
...
void ModelGDistPow::fit(const SimData & sD, const TrtData & tD,
...
void ModelGDistPow::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelGDistPowFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelGDistPowFitObjFn;
    ...
    std::cout << "Error: ModelGDistPow::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
double modelGDistPowFitObjFn (const gsl_vector * x, void * params){
  ModelGDistPowFitData * dat = static_cast<ModelGDistPowFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base-=alpha*std::pow(dat->fD.gDist.at(i*dat->fD.numNodes+j),
				 power);
