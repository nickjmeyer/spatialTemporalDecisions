...
::ModelTimeExpCavesGDistTrendPow(const FixedData & fD)
  : ModelBase("TimeExpCavesGDistTrendPow",genPars(),fD){
}
...
::operator=(const ModelTimeExpCavesGDistTrendPow & m){
  if(this != & m){
    this->ModelTimeExpCavesGDistTrendPow::~ModelTimeExpCavesGDistTrendPow();
    new (this) ModelTimeExpCavesGDistTrendPow(m);
  }
  return *this;
}
...
double
ModelTimeExpCavesGDistTrendPow::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void
ModelTimeExpCavesGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
				    const FixedData & fD,
...
void ModelTimeExpCavesGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
					 const FixedData & fD,
					 const DynamicData & dD,
					 std::vector<double> all){
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeExpCavesGDistTrendPowFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeExpCavesGDistTrendPowFitObjFn;
    ...
    std::cout << "Error: ModelTimeExpCavesGDistTrendPow::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
modelTimeExpCavesGDistTrendPowFitObjFn (const gsl_vector * x, void * params){
  ModelTimeExpCavesGDistTrendPowFitData * dat =
    static_cast<ModelTimeExpCavesGDistTrendPowFitData*> (params);
