...
double
GravityTimeInfExpRCavesModel::tuneTrt(const FixedData & fD,
				      const GravityTimeInfExpRCavesParam & gP){
  int i,j;
  double avgCaves = 0.0;
...
double
GravityTimeInfExpRCavesModel::oneOnOne(const int notNode,
				       const int infNode,
				       const SimData & sD,
				       const TrtData & tD,
				       const FixedData & fD,
				       ...
				       const
				       GravityTimeInfExpRCavesParam & gP) const{
  double base = gP.intcp;
  ...
  double caveTime = fD.rankCaves.at(infNode)*(sD.timeInf.at(infNode)-1.0);
...
void
GravityTimeInfExpRCavesModel::fit(const SimData & sD, const TrtData & tD,
				  const FixedData & fD, const DynamicData & dD,
				  GravityTimeInfExpRCavesParam & mP){
  GravityTimeInfExpRCavesParam mPInit;
...
GravityTimeInfExpRCavesModel::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfExpRCavesModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfExpRCavesModelFitObjFn;
...
GravityTimeInfExpRCavesModelFitData
...
gravityTimeInfExpRCavesModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfExpRCavesModelFitData * dat =
    static_cast<GravityTimeInfExpRCavesModelFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=dat->mP.xi*(std::exp(dat->fD.rankCaves.at(j)*
				       (dat->timeInf.at(t-1).at(j) - 1.0))-1.0);
