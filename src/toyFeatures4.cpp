...
std::vector<double> ToyFeatures4TuneParam::getPar() const{
  return std::vector<double>(0);
}


void ToyFeatures4TuneParam::putPar(const std::vector<double> & par){
  // do nothing!
}
...
template <class M>
void ToyFeatures4<M>::preCompData(const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
...
template <class M>
void ToyFeatures4<M>::getFeatures(const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
				  const DynamicData & dD,
				  M & m){
...
template <class M>
void ToyFeatures4<M>::updateFeatures(const SimData & sD,
				     const TrtData & tD,
				     const FixedData & fD,
				     const DynamicData & dD,
				     M & m){
