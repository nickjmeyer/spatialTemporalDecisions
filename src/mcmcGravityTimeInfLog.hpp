#ifndef GRAVITY_TIME_INF_LOG_MCMC_HPP__
#define GRAVITY_TIME_INF_LOG_MCMC_HPP__
...
class GravityTimeInfLogSamples{
 public:
  int numSamples;
  int numCovar;
...
class GravityTimeInfLogMcmc{
 public:
  ...
  // MCMC samples
  GravityTimeInfLogSamples samples;
  ...
  std::vector<double> timeInfLog; // time infected valued
  ...
  std::vector<double> xiTimeInfLog_cur;
  std::vector<double> xiTimeInfLog_can;
  ...
  inline static void updateCovarBeta(std::vector<double> & covarBeta,
				     ...
				     const int covarInd,
				     const int numCovar);
  inline static void updateXiTimeInfLog(std::vector<double> & xiTimeInfLog,
					const double & scale);
};



inline void GravityTimeInfLogMcmc::updateAlphaW(std::vector<double> & alphaW,
						const double & alphaOld,
						const double & alphaNew,
						const int numNodes){
...
inline
void GravityTimeInfLogMcmc::updateCovarBeta(std::vector<double> & covarBeta,
					    const std::vector<double> & covar,
					    const std::vector<double> & beta,
					    const int numNodes,
					    const int numCovar){
...
inline
void GravityTimeInfLogMcmc
::updateXiTimeInfLog(std::vector<double> & xiTimeInfLog,
		     const double & scale){
  std::for_each(xiTimeInfLog.begin(),xiTimeInfLog.end(),
