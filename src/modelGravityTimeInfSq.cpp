...
double GravityTimeInfSqModel::tuneTrt(const FixedData & fD,
				      const GravityTimeInfSqParam & gP){
  int i,j;
  double avgCaves = 0.0;
...
double GravityTimeInfSqModel::oneOnOne(const int notNode,
				       const int infNode,
				       const SimData & sD,
				       const TrtData & tD,
				       const FixedData & fD,
				       const DynamicData & dD,
				       const GravityTimeInfSqParam & gP) const{
  double base = gP.intcp;
  ...
  base += gP.xi * std::pow(sD.timeInf.at(infNode) - 1.0,2.0);
...
void GravityTimeInfSqModel::fit(const SimData & sD, const TrtData & tD,
				const FixedData & fD, const DynamicData & dD,
				GravityTimeInfSqParam & mP){
  GravityTimeInfSqParam mPInit;
...
void GravityTimeInfSqModel::fit(const SimData & sD, const TrtData & tD,
				const FixedData & fD, const DynamicData & dD,
				GravityTimeInfSqParam & mP,
				const GravityTimeInfSqParam mPInit){
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfSqModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfSqModelFitObjFn;
...
double gravityTimeInfSqModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfSqModelFitData * dat =
    static_cast<GravityTimeInfSqModelFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=dat->mP.xi*std::pow(dat->timeInf.at(t-1).at(j) - 1.0,2.0);
