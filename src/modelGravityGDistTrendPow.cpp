...
ModelGravityGDistTrendPow::ModelGravityGDistTrendPow(const FixedData & fD)
  : ModelBase("GravityGDistTrendPow",genPars(),fD){
}
...
ModelGravityGDistTrendPow & ModelGravityGDistTrendPow
::operator=(const ModelGravityGDistTrendPow & m){
  if(this != & m){
    this->ModelGravityGDistTrendPow::~ModelGravityGDistTrendPow();
    new (this) ModelGravityGDistTrendPow(m);
  }
  return *this;
}
...
double ModelGravityGDistTrendPow::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void ModelGravityGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
				    const FixedData & fD,
...
void ModelGravityGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelGravityGDistTrendPowFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelGravityGDistTrendPowFitObjFn;
    ...
    std::cout << "Error: ModelGravityGDistTrendPow::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
double modelGravityGDistTrendPowFitObjFn (const gsl_vector * x, void * params){
  ModelGravityGDistTrendPowFitData * dat =
    static_cast<ModelGravityGDistTrendPowFitData*> (params);
