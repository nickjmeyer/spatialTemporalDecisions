...
void Model2GravityGDist::fit(const SimData & sD, const TrtData & tD,
...
void Model2GravityGDist::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    Model2GravityGDistFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelGravity2GDistFitObjFn;
    ...
    std::cout << "Error: Model2GravityGDist::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
double modelGravity2GDistFitObjFn (const gsl_vector * x, void * params){
  Model2GravityGDistFitData * dat =
    static_cast<Model2GravityGDistFitData*> (params);
