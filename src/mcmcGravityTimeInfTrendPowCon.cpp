...
void GravityTimeInfTrendPowConSamples::setMode(){
...
void GravityTimeInfTrendPowConSamples::setMean(){
...
void GravityTimeInfTrendPowConSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfTrendPowConSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfTrendPowConSamples::getPar() const {
...
void GravityTimeInfTrendPowConMcmc
...
void GravityTimeInfTrendPowConMcmc::sample(int const numSamples,
					   int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTimeInfTrendPowCon...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfTrendPowConMcmc::ll(){
  int i,j,k;
