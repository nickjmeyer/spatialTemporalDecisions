...
void GravityTimeInfExpCavesSamples::setRand(){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpCavesSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void
GravityTimeInfExpCavesMcmc::load(const std::vector<std::vector<int> > & history,
...
void
GravityTimeInfExpCavesMcmc::sample(int const numSamples, int const numBurn,
				   const bool saveBurn){
...
void GravityTimeInfExpCavesMcmc::sample(int const numSamples, int const numBurn,
...
double GravityTimeInfExpCavesMcmc::ll(){
  int i,j,k;
