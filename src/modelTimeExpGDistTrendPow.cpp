#include "modelTimeExpGDistTrendPow.hpp"
...
static std::vector<ParamBase *> genPars(){
  ...
  pars.push_back(new ParamTrendPow);
...
ModelTimeExpGDistTrendPow::ModelTimeExpGDistTrendPow(const FixedData & fD)
  : ModelBase("TimeExpGDistTrendPow",genPars(),fD){
}
...
ModelTimeExpGDistTrendPow &
ModelTimeExpGDistTrendPow::operator=(const ModelTimeExpGDistTrendPow & m){
  if(this != & m){
    this->ModelTimeExpGDistTrendPow::~ModelTimeExpGDistTrendPow();
    new (this) ModelTimeExpGDistTrendPow(m);
  }
  return *this;
}
...
double
ModelTimeExpGDistTrendPow::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void
ModelTimeExpGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
...
void ModelTimeExpGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
				    const FixedData & fD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeExpGDistTrendPowFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeExpGDistTrendPowFitObjFn;
    ...
    std::cout << "Error: ModelTimeExpGDistTrendPow::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
ModelTimeExpGDistTrendPowFitData
...
modelTimeExpGDistTrendPowFitObjFn (const gsl_vector * x, void * params){
  ModelTimeExpGDistTrendPowFitData * dat =
    static_cast<ModelTimeExpGDistTrendPowFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=trend*std::pow(double(t-1),trendPow);
