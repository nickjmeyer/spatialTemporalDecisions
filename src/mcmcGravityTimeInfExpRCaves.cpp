...
void GravityTimeInfExpRCavesSamples::setMean(){
...
void GravityTimeInfExpRCavesSamples::setRand(){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpRCavesSamples::setPar(const int i){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpRCavesSamples::getPar() const {
...
load(const std::vector<std::vector<int> > & history,
  ...
  covar = fD.covar;
  timeInfExpRCaves.resize(numNodes*T);
  int i,j;
  ...
  for(i = 0; i < numNodes; ++i){
	...
	++val;
      timeInfVal = std::exp(fD.rankCaves.at(i)*(val - 1.0)) - 1.0;
      timeInfExpRCaves.at(i*T + j) = (val > 0) ? timeInfVal : 0.0;
    }
  }
...
void
GravityTimeInfExpRCavesMcmc::sample(int const numSamples, int const numBurn){
...
void GravityTimeInfExpRCavesMcmc::sample(int const numSamples,
					 int const numBurn,
  ...
  xiTimeInfExpRCaves_cur = timeInfExpRCaves;
  updateXiTimeInfExpRCaves(xiTimeInfExpRCaves_cur,xi_cur);
  xiTimeInfExpRCaves_can = xiTimeInfExpRCaves_cur;
  ...
  for(i=0; i<numSamples; ++i){
    ...
    updateXiTimeInfExpRCaves(xiTimeInfExpRCaves_can,xi_can/xi_cur);
    ...
    if(std::log(njm::runif01()) < R){
      ++acc.at(numCovar+XI_);
      xi_cur=xi_can;
      xiTimeInfExpRCaves_cur = xiTimeInfExpRCaves_can;
      ll_cur=ll_can;
    }
    else{
      xi_can=xi_cur;
  ...
  xiTimeInfExpRCaves_can = timeInfExpRCaves;
  updateXiTimeInfExpRCaves(xiTimeInfExpRCaves_can,xi_can);
...
double GravityTimeInfExpRCavesMcmc::ll(){
  int i,j,k;
  ...
  for(i=1; i<T; i++){// loop over time interval that has changed
	...
	for(k=0; k<numNodes; k++){
	  ...
	  if(infHist.at(k*T + i-1)==1){
	    ...
	    baseProb += xiTimeInfExpRCaves_can.at(k*T + i-1);
