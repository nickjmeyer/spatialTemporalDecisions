...
::ModelTimeExpCavesGDistTrendPowCon(const FixedData & fD)
  : ModelBase("TimeExpCavesGDistTrendPowCon",genPars(),fD){
}
...
::operator=(const ModelTimeExpCavesGDistTrendPowCon & m){
  if(this != & m){
    this->ModelTimeExpCavesGDistTrendPowCon
      ::~ModelTimeExpCavesGDistTrendPowCon();
    new (this) ModelTimeExpCavesGDistTrendPowCon(m);
  }
  return *this;
}
...
double
ModelTimeExpCavesGDistTrendPowCon::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void
ModelTimeExpCavesGDistTrendPowCon::fit(const SimData & sD, const TrtData & tD,
				       const FixedData & fD,
...
void ModelTimeExpCavesGDistTrendPowCon::fit(const SimData & sD,
					    const TrtData & tD,
					    const FixedData & fD,
					    const DynamicData & dD,
					    std::vector<double> all){
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeExpCavesGDistTrendPowConFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeExpCavesGDistTrendPowConFitObjFn;
    ...
    std::cout << "Error: ModelTimeExpCavesGDistTrendPowCon"
	      << "::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
...
modelTimeExpCavesGDistTrendPowConFitObjFn (const gsl_vector * x, void * params){
  ModelTimeExpCavesGDistTrendPowConFitData * dat =
    static_cast<ModelTimeExpCavesGDistTrendPowConFitData*> (params);
