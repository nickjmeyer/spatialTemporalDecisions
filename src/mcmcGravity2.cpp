...
void Gravity2Samples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
  betaInfSet.resize(numCovar);
...
std::vector<double> Gravity2Samples::getPar() const {
...
void Gravity2Mcmc::load(const std::vector<std::vector<int> > & history,
...
void Gravity2Mcmc::sample(int const numSamples, int const numBurn,
			  const bool saveBurn){
...
void Gravity2Mcmc::sample(int const numSamples, int const numBurn,
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravity2...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
