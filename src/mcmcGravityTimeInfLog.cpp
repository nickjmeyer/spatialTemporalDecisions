...
void GravityTimeInfLogSamples::setMean(){
...
void GravityTimeInfLogSamples::setRand(){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfLogSamples::setPar(const int i){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfLogSamples::getPar() const {
...
void
GravityTimeInfLogMcmc::load(const std::vector<std::vector<int> > & history,
...
GravityTimeInfLogMcmc::load(const std::vector<std::vector<int> > & history,
  ...
  for(i = 0; i < numNodes; ++i){
	...
	++val;
      timeInfLog.at(i*T + j) = (val > 0) ? std::log(val) : 0.0;
    }
  }
...
void GravityTimeInfLogMcmc::sample(int const numSamples, int const numBurn){
...
void GravityTimeInfLogMcmc::sample(int const numSamples, int const numBurn,
  ...
  xiTimeInfLog_cur = timeInfLog;
  updateXiTimeInfLog(xiTimeInfLog_cur,xi_cur);
  xiTimeInfLog_can = xiTimeInfLog_cur;
  ...
  for(i=0; i<numSamples; ++i){
    ...
    updateXiTimeInfLog(xiTimeInfLog_can,xi_can/xi_cur);
    ...
    if(std::log(njm::runif01()) < R){
      ++acc.at(numCovar+XI_);
      xi_cur=xi_can;
      xiTimeInfLog_cur = xiTimeInfLog_can;
      ll_cur=ll_can;
    }
    else{
      xi_can=xi_cur;
  ...
  xiTimeInfLog_can = timeInfLog;
  updateXiTimeInfLog(xiTimeInfLog_can,xi_can);
...
double GravityTimeInfLogMcmc::ll(){
  int i,j,k;
  ...
  for(i=1; i<T; i++){// loop over time interval that has changed
	...
	for(k=0; k<numNodes; k++){
	  ...
	  if(infHist.at(k*T + i-1)==1){
	    ...
	    baseProb += xiTimeInfLog_can.at(k*T + i-1);
