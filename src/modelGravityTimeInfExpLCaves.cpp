...
double
GravityTimeInfExpLCavesModel::tuneTrt(const FixedData & fD,
				      const GravityTimeInfExpLCavesParam & gP){
  int i,j;
  double avgCaves = 0.0;
...
double
GravityTimeInfExpLCavesModel::oneOnOne(const int notNode,
				       const int infNode,
				       const SimData & sD,
				       const TrtData & tD,
				       const FixedData & fD,
				       ...
				       const
				       GravityTimeInfExpLCavesParam & gP) const{
  double base = gP.intcp;
  ...
  double caveTime = (sD.timeInf.at(infNode)-1.0)/fD.logPropCaves.at(infNode);
...
void
GravityTimeInfExpLCavesModel::fit(const SimData & sD, const TrtData & tD,
				  const FixedData & fD, const DynamicData & dD,
				  GravityTimeInfExpLCavesParam & mP){
  GravityTimeInfExpLCavesParam mPInit;
...
GravityTimeInfExpLCavesModel::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfExpLCavesModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfExpLCavesModelFitObjFn;
...
GravityTimeInfExpLCavesModelFitData
...
gravityTimeInfExpLCavesModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfExpLCavesModelFitData * dat =
    static_cast<GravityTimeInfExpLCavesModelFitData*> (params);
