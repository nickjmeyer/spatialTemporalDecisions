#include "modelTimeExpGDistTrend.hpp"
...
ModelTimeExpGDistTrend::ModelTimeExpGDistTrend(const FixedData & fD)
  : ModelBase("TimeExpGDistTrend",genPars(),fD){
}


ModelTimeExpGDistTrend::ModelTimeExpGDistTrend(const
...
ModelTimeExpGDistTrend::operator=(const ModelTimeExpGDistTrend & m){
  if(this != & m){
    this->ModelTimeExpGDistTrend::~ModelTimeExpGDistTrend();
    new (this) ModelTimeExpGDistTrend(m);
  }
  return *this;
}
...
double
ModelTimeExpGDistTrend::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void
ModelTimeExpGDistTrend::fit(const SimData & sD, const TrtData & tD,
...
void ModelTimeExpGDistTrend::fit(const SimData & sD, const TrtData & tD,
				 const FixedData & fD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeExpGDistTrendFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeExpGDistTrendFitObjFn;
    ...
    std::cout << "Error: ModelTimeExpGDistTrend::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
modelTimeExpGDistTrendFitObjFn (const gsl_vector * x, void * params){
  ModelTimeExpGDistTrendFitData * dat =
    static_cast<ModelTimeExpGDistTrendFitData*> (params);
