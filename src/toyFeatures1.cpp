...
std::vector<double> ToyFeatures1TuneParam::getPar() const{
  return std::vector<double>(0);
}


void ToyFeatures1TuneParam::putPar(const std::vector<double> & par){
  // do nothing!
}
...
template class ToyFeatures1<GravityModel,GravityParam>;

template class ToyFeatures1<EbolaModel,EbolaParam>;
...
void ToyFeatures1<M,MP>::getFeatures(const SimData & sD,
  ...
  if(!noneTrt){
    ...
    getSubGraph(fD.numNodes,&networkNoTrt,&subgraphNoTrt,fD.subGraphKval);
  }
  ...
  if(!noneTrt){
	...
	if(1.0/fD.logDist.at(node0*fD.numNodes + sD.infected.at(j)) > minDist)
      ...
      subgraphNode=fD.subGraphK.at(node0);
      if(subgraphNode>0)
	...
	if(1.0/fD.logDist.at(node0*fD.numNodes + sD.infected.at(j)) > minDist)
	  minDist = 1.0/fD.logDist.at(node0*fD.numNodes + sD.infected.at(j));
      notFeat(i,featNum) = minDist*fD.subGraphK.at(node0)/fD.subGraphKmax;
    }
    for(i=0; i<sD.numInfected; i++){
      node0=sD.infected.at(i);
      minDist=0;
	...
	if(1.0/fD.logDist.at(node0*fD.numNodes + sD.notInfec.at(j)) > minDist)
	  minDist = 1.0/fD.logDist.at(node0*fD.numNodes + sD.notInfec.at(j));
      infFeat(i,featNum) = minDist*fD.subGraphK.at(node0)/fD.subGraphKmax;
    }
  }
