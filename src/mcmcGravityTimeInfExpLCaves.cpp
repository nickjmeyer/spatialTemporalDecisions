...
void GravityTimeInfExpLCavesSamples::setMean(){
...
void GravityTimeInfExpLCavesSamples::setRand(){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpLCavesSamples::setPar(const int i){
  intcpSet = alphaSet = powerSet = xiSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpLCavesSamples::getPar() const {
...
load(const std::vector<std::vector<int> > & history,
  ...
  covar = fD.covar;
  timeInfExpLCaves.resize(numNodes*T);
  int i,j;
  ...
  for(i = 0; i < numNodes; ++i){
	...
	++val;
      timeInfVal = std::exp((val - 1.0)/fD.logPropCaves.at(i)) - 1.0;
      timeInfExpLCaves.at(i*T + j) = (val > 0) ? timeInfVal : 0.0;
    }
  }
...
void
GravityTimeInfExpLCavesMcmc::sample(int const numSamples, int const numBurn){
...
void GravityTimeInfExpLCavesMcmc::sample(int const numSamples,
					 int const numBurn,
  ...
  xiTimeInfExpLCaves_cur = timeInfExpLCaves;
  updateXiTimeInfExpLCaves(xiTimeInfExpLCaves_cur,xi_cur);
  xiTimeInfExpLCaves_can = xiTimeInfExpLCaves_cur;
  ...
  for(i=0; i<numSamples; ++i){
    ...
    updateXiTimeInfExpLCaves(xiTimeInfExpLCaves_can,xi_can/xi_cur);
    ...
    if(std::log(njm::runif01()) < R){
      ++acc.at(numCovar+XI_);
      xi_cur=xi_can;
      xiTimeInfExpLCaves_cur = xiTimeInfExpLCaves_can;
      ll_cur=ll_can;
    }
    else{
      xi_can=xi_cur;
  ...
  xiTimeInfExpLCaves_can = timeInfExpLCaves;
  updateXiTimeInfExpLCaves(xiTimeInfExpLCaves_can,xi_can);
...
double GravityTimeInfExpLCavesMcmc::ll(){
  int i,j,k;
  ...
  for(i=1; i<T; i++){// loop over time interval that has changed
	...
	for(k=0; k<numNodes; k++){
	  ...
	  if(infHist.at(k*T + i-1)==1){
	    ...
	    baseProb += xiTimeInfExpLCaves_can.at(k*T + i-1);
