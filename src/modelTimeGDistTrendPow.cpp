#include "modelTimeGDistTrendPow.hpp"
...
ModelTimeGDistTrendPow::ModelTimeGDistTrendPow(const FixedData & fD)
  : ModelBase("TimeGDistTrendPow",genPars(),fD){
}
...
ModelTimeGDistTrendPow & ModelTimeGDistTrendPow
::operator=(const ModelTimeGDistTrendPow & m){
  if(this != & m){
    this->ModelTimeGDistTrendPow::~ModelTimeGDistTrendPow();
    new (this) ModelTimeGDistTrendPow(m);
  }
  return *this;
}
...
double ModelTimeGDistTrendPow::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void ModelTimeGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
...
void ModelTimeGDistTrendPow::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeGDistTrendPowFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeGDistTrendPowFitObjFn;
    ...
    std::cout << "Error: ModelTimeGDistTrendPow::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
double modelTimeGDistTrendPowFitObjFn (const gsl_vector * x, void * params){
  ModelTimeGDistTrendPowFitData * dat =
    static_cast<ModelTimeGDistTrendPowFitData*> (params);
