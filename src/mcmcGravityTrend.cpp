...
enum parInd{INTCP_=0,ALPHA_=1,POWER_=2,TREND_=3,TRTP_=4,TRTA_=5};



void GravityTrendSamples::setMode(){
...
void GravityTrendSamples::setMean(){
  intcpSet = alphaSet = powerSet = trendSet = trtPreSet = trtActSet = 0.0;
...
void GravityTrendSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTrendSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTrendSamples::getPar() const {
...
void GravityTrendMcmc::load(const std::vector<std::vector<int> > & history,
...
void GravityTrendMcmc::sample(int const numSamples, int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTrend...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
