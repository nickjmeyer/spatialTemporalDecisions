...
void GravityTimeInfExpCavesTrendSamples::setMode(){
...
void GravityTimeInfExpCavesTrendSamples::setMean(){
...
void GravityTimeInfExpCavesTrendSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpCavesTrendSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpCavesTrendSamples::getPar() const {
...
void
GravityTimeInfExpCavesTrendMcmc::sample(int const numSamples,
					int const numBurn,
const bool saveBurn){
...
void GravityTimeInfExpCavesTrendMcmc::sample(int const numSamples,
					     int const numBurn,
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("GravityTimeInfExpCavesTrend...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfExpCavesTrendMcmc::ll(){
  int i,j,k;
