#include "modelTimeGDistTrendPowCon.hpp"
...
ModelTimeGDistTrendPowCon::ModelTimeGDistTrendPowCon(const FixedData & fD)
  : ModelBase("TimeGDistTrendPowCon",genPars(),fD){
}
...
ModelTimeGDistTrendPowCon & ModelTimeGDistTrendPowCon
::operator=(const ModelTimeGDistTrendPowCon & m){
  if(this != & m){
    this->ModelTimeGDistTrendPowCon::~ModelTimeGDistTrendPowCon();
    new (this) ModelTimeGDistTrendPowCon(m);
  }
  return *this;
}
...
double ModelTimeGDistTrendPowCon::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void ModelTimeGDistTrendPowCon::fit(const SimData & sD, const TrtData & tD,
				    const FixedData & fD,
...
void ModelTimeGDistTrendPowCon::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeGDistTrendPowConFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeGDistTrendPowConFitObjFn;
    ...
    std::cout << "Error: ModelTimeGDistTrendPowCon::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
ModelTimeGDistTrendPowConFitData
::ModelTimeGDistTrendPowConFitData
(const ModelTimeGDistTrendPowCon & m,
...
double modelTimeGDistTrendPowConFitObjFn (const gsl_vector * x, void * params){
  ModelTimeGDistTrendPowConFitData * dat =
    static_cast<ModelTimeGDistTrendPowConFitData*> (params);
