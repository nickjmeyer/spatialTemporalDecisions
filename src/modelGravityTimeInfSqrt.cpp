...
double GravityTimeInfSqrtModel::tuneTrt(const FixedData & fD,
					const GravityTimeInfSqrtParam & gP){
  int i,j;
  double avgCaves = 0.0;
...
double
GravityTimeInfSqrtModel::oneOnOne(const int notNode,
				  const int infNode,
				  const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
				  ...
				  const
				  GravityTimeInfSqrtParam & gP) const{
  double base = gP.intcp;
  ...
  base += gP.xi * std::sqrt(sD.timeInf.at(infNode) - 1.0);
...
void GravityTimeInfSqrtModel::fit(const SimData & sD, const TrtData & tD,
				  const FixedData & fD, const DynamicData & dD,
				  GravityTimeInfSqrtParam & mP){
  GravityTimeInfSqrtParam mPInit;
...
void GravityTimeInfSqrtModel::fit(const SimData & sD, const TrtData & tD,
				  const FixedData & fD, const DynamicData & dD,
				  GravityTimeInfSqrtParam & mP,
				  const GravityTimeInfSqrtParam mPInit){
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfSqrtModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfSqrtModelFitObjFn;
...
GravityTimeInfSqrtModelFitData
...
double gravityTimeInfSqrtModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfSqrtModelFitData * dat =
    static_cast<GravityTimeInfSqrtModelFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=dat->mP.xi*std::sqrt(dat->timeInf.at(t-1).at(j) - 1.0);
