#include "modelTimeGDistTrend.hpp"
...
ModelTimeGDistTrend::ModelTimeGDistTrend(const FixedData & fD)
  : ModelBase("TimeGDistTrend",genPars(),fD){
}
...
ModelTimeGDistTrend & ModelTimeGDistTrend
::operator=(const ModelTimeGDistTrend & m){
  if(this != & m){
    this->ModelTimeGDistTrend::~ModelTimeGDistTrend();
    new (this) ModelTimeGDistTrend(m);
  }
  return *this;
}
...
double ModelTimeGDistTrend::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
void ModelTimeGDistTrend::fit(const SimData & sD, const TrtData & tD,
...
void ModelTimeGDistTrend::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE || fitType == MLES){
    ...
    ModelTimeGDistTrendFitData dat(*this,all,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&modelTimeGDistTrendFitObjFn;
    ...
    std::cout << "Error: ModelTimeGDistTrend::fit(): MCMC not setup"
	      << std::endl;
    throw(1);
  }
  else{
...
double modelTimeGDistTrendFitObjFn (const gsl_vector * x, void * params){
  ModelTimeGDistTrendFitData * dat =
    static_cast<ModelTimeGDistTrendFitData*> (params);
