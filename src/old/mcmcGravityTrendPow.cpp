...
enum parInd{INTCP_=0,ALPHA_=1,POWER_=2,TREND_=3,TRENDPOW_=4,TRTP_=5,TRTA_=6};



void GravityTrendPowSamples::setMode(){
...
void GravityTrendPowSamples::setMean(){
...
void GravityTrendPowSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTrendPowSamples::getPar() const {
...
void GravityTrendPowMcmc::load(const std::vector<std::vector<int> > & history,
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTrendPow...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
