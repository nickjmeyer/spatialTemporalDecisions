...
enum parInd{INTCP_=0,ALPHA_=1,POWER_=2,TREND_=3,TRENDPOWCON_=4,TRTP_=5,TRTA_=6};



void GravityTrendPowConSamples::setMode(){
...
void GravityTrendPowConSamples::setMean(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet =
...
void GravityTrendPowConSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTrendPowConSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTrendPowConSamples::getPar() const {
...
void GravityTrendPowConMcmc::sample(int const numSamples, int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTrendPowCon...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTrendPowConMcmc::ll(){
  int i,j,k;
