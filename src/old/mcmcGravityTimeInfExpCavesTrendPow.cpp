...
void GravityTimeInfExpCavesTrendPowSamples::setMode(){
  ...
  trendPowSet = DensityEst(trendPow).max().second;
...
void GravityTimeInfExpCavesTrendPowSamples::setMean(){
...
void GravityTimeInfExpCavesTrendPowSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpCavesTrendPowSamples::getPar() const {
...
void
GravityTimeInfExpCavesTrendPowMcmc
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("GravityTimeInfExpCavesTrendPow...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfExpCavesTrendPowMcmc::ll(){
  int i,j,k;
