...
void GravityTimeInfExpTrendPowSamples::setMode(){
...
void GravityTimeInfExpTrendPowSamples::setMean(){
...
void GravityTimeInfExpTrendPowSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpTrendPowSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trendPowSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpTrendPowSamples::getPar() const {
...
void GravityTimeInfExpTrendPowMcmc::sample(int const numSamples,
					   int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTimeInfExpTrendPow...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfExpTrendPowMcmc::ll(){
  int i,j,k;
