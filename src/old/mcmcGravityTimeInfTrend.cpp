...
void GravityTimeInfTrendSamples::setMode(){
...
void GravityTimeInfTrendSamples::setMean(){
...
void GravityTimeInfTrendSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfTrendSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfTrendSamples::getPar() const {
...
void GravityTimeInfTrendMcmc::sample(int const numSamples, int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTimeInfTrend...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfTrendMcmc::ll(){
  int i,j,k;
