...
void GravityTimeInfExpTrendPowConSamples::setMode(){
...
void GravityTimeInfExpTrendPowConSamples::setMean(){
...
void GravityTimeInfExpTrendPowConSamples::setRand(){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
void GravityTimeInfExpTrendPowConSamples::setPar(const int i,const bool fromBurn){
  intcpSet = alphaSet = powerSet = trendSet = trendPowConSet = xiSet =
    trtPreSet = trtActSet = 0.0;
  betaSet.resize(numCovar);
  std::fill(betaSet.begin(),betaSet.end(),0.0);
...
std::vector<double> GravityTimeInfExpTrendPowConSamples::getPar() const {
...
void
GravityTimeInfExpTrendPowConMcmc
...
void GravityTimeInfExpTrendPowConMcmc::sample(int const numSamples,
					   int const numBurn,
const bool saveBurn){
...
samples.llBurn.reserve(numBurn);
  ...
  for(i=0; i<numSamples; ++i){
    if(display && i%displayOn==0){
      printf("McmcGravityTimeInfExpTrendPowCon...%6s: %6d\r","iter",i);
      fflush(stdout);
    }
...
double GravityTimeInfExpTrendPowConMcmc::ll(){
  int i,j,k;
