...
void GravityTimeInfExpModel::load(const SimData & sD,
				  const TrtData & tD,
				  const FixedData & fD,
...
void GravityTimeInfExpModel::infProbs(const SimData & sD,
				      const TrtData & tD,
				      const FixedData & fD,
				      const DynamicData & dD){
  mP.infProbs.clear();
...
void GravityTimeInfExpModel::update(const SimData & sD,
				    const TrtData & tD,
				    const FixedData & fD,
				    const DynamicData & dD){
...
double GravityTimeInfExpModel::tuneTrt(const FixedData & fD){
  int i,j;
  double avgCaves = 0.0;
...
double
GravityTimeInfExpModel::oneOnOne(const int notNode,
				 const int infNode,
				 const SimData & sD,
				 const TrtData & tD,
				 const FixedData & fD,
  ...
  base += mP.xi * (std::exp(sD.timeInf.at(infNode)-1.0)-1.0);
...
void GravityTimeInfExpModel::fit(const SimData & sD, const TrtData & tD,
...
void GravityTimeInfExpModel::fit(const SimData & sD, const TrtData & tD,
  ...
  if(fitType == MLE){
    ...
    GravityTimeInfExpModelFitData dat(*this,mPInit,sD,fD,history);
    ...
    minex_func.n=dim;
    minex_func.f=&gravityTimeInfExpModelFitObjFn;
...
double gravityTimeInfExpModelFitObjFn (const gsl_vector * x, void * params){
  GravityTimeInfExpModelFitData * dat =
    static_cast<GravityTimeInfExpModelFitData*> (params);
  ...
  for(t=1; t<time; t++){
	...
	for(j=0; j<dat->fD.numNodes; j++){
	  if(dat->history.at(t-1).at(j) >= 2){
	    ...
	    base+=dat->mP.xi*(std::exp(dat->timeInf.at(t-1).at(j) - 1.0)-1.0);
